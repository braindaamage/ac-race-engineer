...
Spring stiffness controls how quickly the body rolls in response to lateral acceleration and how much body roll ultimately occurs at a given lateral G. Stiffer springs resist body roll more effectively, keeping tyre camber angles closer to their static values and reducing the amplitude of load transfer through the tyre contact patch. A stiffer front spring increases front roll stiffness and shifts the lateral load transfer distribution toward the front axle, tending to increase understeer. A stiffer rear spring has the opposite directional effect. Springs also interact with longitudinal weight transfer: stiffer springs resist pitch, which can affect brake balance feel and traction at the driven axle.
...
The differential controls how torque is distributed between the driven wheels and how much rotational speed difference is permitted between them. Under power at corner exit, a locked or high-ramp-angle differential applies torque more equally to both driven wheels. On a rear-wheel-drive car, this resists the inner wheel spinning freely and can create an oversteering moment if the outer wheel is near its traction limit. A more open differential allows the inner wheel to spin with less resistance, which can reduce corner exit oversteer. Differential preload affects low-speed behaviour; ramp angles (acceleration and deceleration) affect power-on and power-off transitions. On front-wheel-drive cars the interactions are reversed, with locking effects contributing to understeer.
...
- **drivetrain.md** — Differential types and settings (open, limited-slip, locking), ramp angles and preload, torque vectoring where applicable, and their effects on driven-axle traction and corner exit balance.
