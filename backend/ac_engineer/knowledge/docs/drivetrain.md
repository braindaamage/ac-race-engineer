...
An LSD limits the speed difference between the two driven wheels by imposing a locking torque. The behavior differs by ramp configuration:

- **1-way LSD**: Locks only under power (acceleration ramps engaged). Under coast or braking, it behaves like an open differential. Suited to circuits where corner entry stability is prioritized; the free coast behavior allows the car to rotate without drivetrain interference on turn-in.
- **1.5-way LSD**: Locks under power with high ramp angles and applies a weaker, partial lock under coast. Offers a compromise between corner entry rotation (less coast lock than a 2-way) and some stability on trailing throttle.
- **2-way LSD**: Locks equally under both power and coast. Provides maximum stability and traction but resists yaw rotation in both phases. Can produce understeer on corner entry when the differential resists the inside wheel slowing relative to the outside.
...
**Power and Coast Ramp Angles**
Ramp angles (measured in degrees) determine how aggressively the differential locks as a function of torque magnitude. A steeper power ramp angle produces a quicker, stronger lockup during acceleration, pushing the car into understeer sooner on corner exit. A shallower power ramp allows more wheel speed differentiation before lock engages, preserving rotation deeper into the corner exit. Coast ramp angles mirror this logic: steeper coast ramps lock the diff under engine braking, stabilizing the rear but resisting rotation on corner entry.
